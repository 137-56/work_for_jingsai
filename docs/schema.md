...
| `category` | string | ⬜ | **规则分类**（2026-09-23 新增，供报告统计"五类各几条"）。取值见 §2.6 |
...
**两个只能靠 `scripts/check_rules.py` 拦下的坑（写进契约备查）**：

1. **`trigger` 不得用 `recipe_id` / `schema_version` 之类的标识字段当"恒真开关"**。
   靠标识符里有没有某个字母来控制规则是否生效，会因取值不同而**静默失效**（规则压根不被求值）。
   需要"恒定触发"时，正确写法是对**语义字段**用 `contains ""`（语义 = "该字段存在时"）。
2. **`domain_check` 类规则的告警建议必须直接写进 `action.message`，不能依赖 `{suggestion}`**。
   引擎按**命中值**查 `suggestions`（如 `suggestions["#FF00FF"]`），而越界值无法穷举 → 永远落到兜底文案。
   （`forbidden_terms` 不受此限，因为命中值就是规则里写明的禁用词。）
...
### 2.6 规则覆盖要求（目标：25 条，**已达成，2026-09-23**）

五类各不少于 4 条。**实际实现为五类各 5 条**，分类落在 `category` 字段上：

| `category` 取值 | 说明 | 实际用的 `constraint.type` | 条数 |
|---|---|---|---|
| `场合-意象冲突` | 某场合下禁用某类意象 | `forbidden_terms` | 5 |
| `寓意与要素完整性` | 配方的关键语义字段须给出且为完整词条 | `required_evidence` | 5 |
| `纹样等级与僭越` | 礼制等级约束使用场景 | `forbidden_terms` | 5 |
| `结构依赖` | 字段间依赖关系 | `dependency` | 5 |
| `色彩与取值域规程` | 取色与取值须在允许域内 | `domain_check` / `forbidden_terms` | 5 |

> **与原目标的差异（如实记录）**：
> - 原第 2 类「谐音可推导」→ 实为「寓意与要素完整性」。
>   原因：`required_evidence` 只能查"字段给没给、每项是否 ≥2 字"，**查不了推导关系**；
>   "可推导"由 `结构依赖` 类的 R018（有寓意必须有构成元素）近似表达。
> - 原第 4 类「载体—密度」→ 实为「结构依赖」。
>   原因：**配方里没有载体字段**（载体是样本的属性，不在 recipe 中），
>   且 `get_by_path()` 不支持数组索引，取不到 `provenance[].carrier`，故无法按原定义实现。
> - 原第 5 类「色彩规程」→ 扩为「色彩与取值域规程」。
>   原因：`domain_check` 同时承载色彩色域与场合取值域两类检查，原名过窄。

**每条 `source` 必须非空。** 规则条数与文献依据，直接对应报告的创新性评分。
**写完必须跑 `scripts/check_rules.py`**（结构与路径校验）和 `scripts/test_culture_rules.py`（检出率与误报率）。
